export class M2mService {
    constructor() {
        this.PARTNER_ID = '388900';
        this.MAKE_WEBHOOK_URL = 'https://hook.eu1.make.com/ywq8k4s6gyaj0sv9b9ci603l1uvzdnnc';
    }
    async getAllOffers(categories) {
        try {
    ...
    async getBestOffer(amount, term, months, categories, incomeType, ageGroup, excludedBanks, debtAmount, creditCardLimit) {
        try {
            const response = await fetch(this.MAKE_WEBHOOK_URL, {
                method: 'POST',
                headers: {
    ...
    async getLoanOffers(amount, term, interestRate) {
        try {
            const response = await fetch(this.MAKE_WEBHOOK_URL, {
                method: 'POST',
                headers: {

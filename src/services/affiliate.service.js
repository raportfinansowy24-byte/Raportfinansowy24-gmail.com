export class AffiliateService {
    ...
    isFraud(text) {
        if (!this.config.fraud_protection.enabled)
            return false;
        if (text.length < this.config.fraud_protection.min_query_length)
            return true;
        const lowerText = text.toLowerCase();
        if (this.config.fraud_protection.spam_keywords.some(k => lowerText.includes(k)))
            return true;
        // Velocity check
        const now = Date.now();
        ...
        if (recentClicks.length >= this.config.fraud_protection.velocity_click_limit.max_clicks)
            return true;
        return false;
    }
    matchOffer(text) {
        if (this.isFraud(text))
            return null;
        const lowerText = text.toLowerCase();
